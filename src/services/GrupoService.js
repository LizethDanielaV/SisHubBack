...
async function crearGrupo(
  ...
  try {
    const nuevoGrupo = await Grupo.create({
      ...
      periodo: periodo,
      anio: anio,
      clave_acceso: clave_acceso ? clave_acceso : generarClaveAcceso(),
      estado: true,
    });
  ...
  try {
    ...
    for (const fila of filasUnicas) {
      ...
      try {
        ...
        // Validar si el grupo ya existe (llave compuesta lógica)
        const grupoExistente = await Grupo.findOne({
          where: {
            codigo_materia: codigo_materia,
            nombre: nombre_grupo,
    ...
    return {
      mensaje: "Carga de grupos completada",
