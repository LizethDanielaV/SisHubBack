...
async function actualizarEstado(req, res) {
  try {
    ...
    const grupoActualizado = await GrupoService.actualizarEstado(
      codigo_materia,
      nombre,
      periodo,
      anio,
...
async function generarCodigoQR(req, res) {
  try {
    ...
    const resultado = await GrupoService.generarCodigoQR(
      codigo_materia,
      nombre,
      periodo,
      anio
...
async function obtenerClaveYCodigoQR(req, res) {
  try {
    ...
    const resultado = await GrupoService.obtenerClaveYCodigoQR(
      codigo_materia,
      nombre,
      periodo,
      anio
    ...
    res.status(error.statusCode || 500).json({
      message:
        error.message || "Error interno al obtener la clave y el código QR",
    });
  }
}
...
async function listarGruposHabilitadosPorMateria(req, res) {
  try {
    const { codigo_materia } = req.params;
    const grupos = await GrupoService.listarGruposHabilitadosPorMateria(
      codigo_materia
    );
    res.status(200).json(grupos);
  } catch (error) {
