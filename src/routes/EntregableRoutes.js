...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    else if (tipo === 'REPOSITORIO') {
      const allowedMimes = ['application/zip', 'application/x-zip-compressed'];

      if (allowedMimes.includes(file.mimetype)) {
        cb(null, true);
      } else {
        cb(new Error('Solo se permiten archivos ZIP para repositorios'));
      }
    }
    else if (tipo === 'IMAGEN') {
...
router.put("/deshabilitar/:id_entregable", EntregableController.deshabilitarEntregable);
